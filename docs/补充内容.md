...
在使用`PyCharm`进行编程的时候，PyCharm会自动提示你函数传入的参数名字，你可能会看到下面的内容：

![img_1.png](_static/img.png)

`key`和`reverse`可以通过官方文档 [.sort()](https://docs.python.org/zh-cn/3/library/stdtypes.html#list.sort)
来了解参数的意思，但在这两个参数之前还存在两个参数：`self`和`*`。

其中`self`和`Python 类方法`
有关，不属于目前的学习阶段内容，这里就不做解释了，后面的小组会讲😀，如果想提前了解可以看 [这里](https://docs.python.org/zh-cn/3/faq/programming.html#what-is-self) 。

现在就剩下`*`这个参数了，但如果你还记得python变量名的写法的话，你会发现参数是不能以 "\*" 号开头的，更不用说单个 "*"
出现的情况了。所以它并不算一个参数，事实上它更像一个**标志位**。
